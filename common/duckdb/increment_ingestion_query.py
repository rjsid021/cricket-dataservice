...
from 
  {DB_NAME}.Matches
where load_timestamp = '{str(load_timestamp)}' ALLOW FILTERING;
'''
...
select 
  ...
  threes 
from 
  {DB_NAME}.MatchBattingCard
where load_timestamp = '{str(load_timestamp)}' ALLOW FILTERING;
'''

GET_BOWL_CARD_DATA = f'''
select 
  match_id, 
  innings, 
  team_id, 
...
where 
  bdf.innings not in (3, 4) and bdf.load_timestamp = '{str(load_timestamp)}';
'''

TEAMS_AGGREGATED_SQL = f'''
select 
  season, 
  match_id, 
  batsman_team_id as team_id, 
  batsman_team_name as team1, 
  bowler_team_name as team2, 
  venue, 
  innings, 
  cast(
    sum(
...
from 
  join_data
where load_timestamp = '{str(load_timestamp)}'
group by 
  season, 
  venue, 
  match_id, 
...
select 
  ...
  md.venue, 
  mp.striker, 
  pd.player_name as striker_name, 
  mp.striker_runs, 
  mp.striker_balls, 
  mp.partnership_total, 
  mp.striker_balls + mp.non_striker_balls as partnership_balls, 
  mp.non_striker, 
  md.competition_name, 
  pd1.player_name as non_striker_name, 
  mp.non_striker_runs, 
  mp.non_striker_balls, 
  case when mp.team_id = md.winning_team then 'Winning' when winning_team =-1 then 'No Result' else 'Losing' end as winning_type, 
  tds.team_name as striker_current_team, 
  tdns.team_name as non_striker_current_team, 
  md.match_date_form as match_date 
from 
  partnership_base_data mp 
  inner join teams_data td on (mp.team_id = td.team_id) 
  inner join matches_df md on (mp.match_id = md.match_id) 
  left join players_data_df pd on (
    mp.striker = pd.player_id 
    and pd.competition_name = md.competition_name
  ) 
  left join players_data_df pd1 on (
    mp.non_striker = pd1.player_id 
    and pd1.competition_name = md.competition_name
  ) 
  left join teams_data tds on (pd.team_id = tds.team_id) 
  left join teams_data tdns on (pd1.team_id = tdns.team_id)
where mp.load_timestamp = '{str(load_timestamp)}';
'''

BOWLER_OVERWISE_SQL = f'''
select 
  match_name, 
  match_id, 
...
from 
  join_data 
where 
  innings not in (3, 4) and load_timestamp = '{str(load_timestamp)}' 
group by 
  match_id, 
  over_number, 
  player_id, 
...
select 
  ...
  jd.winning_team, 
  jd.load_timestamp as load_timestamp,
  jd.match_phase, 
  jd.pitch_type, 
  jd.bat_current_team, 
...
where 
  jd.innings not in (3, 4) and jd.load_timestamp = '{str(load_timestamp)}'
group by 
...
select 
  ...
  batting_type, 
  bowling_type, 
  case when bowler_team_id = winning_team then 'WIN' when winning_team =-1 then 'NO RESULT' else 'LOSS' end as match_decision, 
  match_date, 
  winning_type, 
  bowler_team_name as team_name, 
  batsman_team_name as team2, 
  cast(
    (
      count(ball_number)-(
        sum(is_Wide)+ sum(is_no_ball)
  ...
  cast(
    sum(
      case when is_leg_bye = 1 then 0 else runs end
    ) as int
  ) as runs, 
  bowler_skill as player_skill, 
  bowler_team_id as team_id, 
...
POSITIONWISE_BOWLER_OVER = f'''
select 
  match_id, 
  bowler_id as player_id, 
  batting_position, 
  over_number as overs, 
  season, 
  innings, 
  venue, 
  batting_type, 
...
POSITIONWISE_BATSMAN_OVER = f'''
select 
  match_id, 
  season, 
  venue, 
  batsman_team_id as team_id, 
  batsman_skill as player_skill, 
  case when batting_position in (1, 2) then 'Opening' else batting_position end as batting_position, 
  innings, 
  winning_type, 
  bowling_type, 
  batting_type, 
  ...
  case when bowler_team_id = winning_team then 'Lose' when winning_team =-1 then 'No Result' else 'Win' end as result,  
  cast(
    sum(ball_runs) as int
  ) as runs, 
  competition_name 
from 
  join_data
...
select 
  ...
  md.match_date, 
  md.team1, 
  td.team_name as team1_name, 
  td1.team_name as team2_name, 
  md.team2, 
  case when team1 = md.winning_team then 'Winning' when winning_team =-1 then 'No Result' else 'Losing' end as winning_type, 
  md.winning_team as winning_team_id, 
  td2.team_short_name as winning_team, 
  td.team_short_name as team1_short_name, 
  td1.team_short_name as team2_short_name, 
  md.competition_name, 
  md.team1_score, 
...
from 
  matches_df md 
  left join teams_data td on (md.team1 = td.team_id) 
  left join teams_data td1 on (md.team2 = td1.team_id) 
  left join teams_data td2 on (md.winning_team = td2.team_id) 
  left join venue_data vd on (vd.venue_id = md.venue)
where md.load_timestamp = '{str(load_timestamp)}';
'''
...
select 
  ...
  cast(
    sum(case when runs = 1 then 1 else 0 end) as int
  ) as num_singles, 
  cast(
    sum(case when runs = 2 then 1 else 0 end) as int
  ) as num_doubles, 
  cast(
    sum(case when runs = 3 then 1 else 0 end) as int
  ) as num_triples, 
  cast(
    sum(
      coalesce(fours, 0)
  ...
  cast(
    sum(is_batsman_out) as int
  ) as is_batsman_out, 
  batting_phase as phase, 
  season as year, 
  innings, 
  batting_type, 
  bowling_type, 
  ...
  cast(bowler_id as int) as bowler_id, 
...
select 
  ...
  cast(
    sum(case when runs = 1 then 1 else 0 end) as int
  ) as num_singles_conceded, 
  cast(
    sum(case when runs = 2 then 1 else 0 end) as int
  ) as num_doubles_conceded, 
  cast(
    sum(case when runs = 3 then 1 else 0 end) as int
  ) as num_triples_conceded, 
  cast(
    sum(
      coalesce(fours, 0)
    ) as int
  ) as num_fours_conceded, 
  cast(
    sum(
      coalesce(sixes, 0)
    ) as int
  ) as num_sixes_conceded, 
  cast(
    sum(dot_balls) as int
  ) as num_dots_bowled, 
  cast(
  ...
  cast(
    sum(wickets) as int
  ) as total_wickets_taken, 
  match_id, 
  batting_phase as phase, 
  season as year, 
  cast(batsman_id as int) as batsman_id, 
  match_name, 
  venue, 
  innings, 
  batting_type, 
...
select 
  ...
  ball_no, 
  date_name, 
  delivery_runup_distance, 
  peak_player_load, 
  raw_peak_roll, 
  raw_peak_yaw, 
  team_name 
from 
  {DB_NAME}.fitnessGPSBallData 
where 
  is_match_fielding = 1
'''

GET_CONTRIBUTION_SCORE_DATA = f'''
select 
  match_date, 
  game_id, 
  season, 
  ...
  load_timestamp as load_timestamp
from 
  {DB_NAME}.contributionScore 
where load_timestamp = '{str(load_timestamp)}' ALLOW FILTERING;
'''

GET_AGG_CONTRIBUTION_DATA = f'''
select 
  season, 
  is_won, 
  competition_name, 
  ...
  sum(
    coalesce(batting_contribution_score, 0)
  ) as batting_contribution_score, 
  sum(
    coalesce(balls_faced, 0)
  ) as balls_played, 
  sum(runs_scored) as runs_scored, 
  sum(
    coalesce(bat_innings, 0)
  ) as bat_innings, 
  sum(
    coalesce(batting_consistency_score, 0)
  ) as batting_consistency_score, 
  sum(
    coalesce(bowling_contribution_score, 0)
  ) as bowling_contribution_score, 
  sum(overall_economy) as overall_economy, 
  cast(
    sum(
      coalesce(wickets_taken, 0)
    ) as int
  ) as wickets_taken, 
  sum(
    coalesce(bowl_innings, 0)
  ) as bowl_innings, 
 ...
 sum(actual_death_over_balls) as actual_death_over_balls,
  sum(
    coalesce(bowling_consistency_score, 0)
  ) as bowling_consistency_score, 
  sum(
    coalesce(overall_contribution_score, 0)
  ) as overall_contribution_score, 
  sum(
    coalesce(overall_consistency_score, 0)
  ) as overall_consistency_score, 
  sum(
    coalesce(
      bowl_powerplay_contribution_score, 
      0
    )
  ) as bowl_powerplay_contribution_score, 
  sum(
    coalesce(
      bowl_7_10_overs_contribution_score, 
      0
    )
  ) as bowl_7_10_overs_contribution_score, 
  sum(
    coalesce(
      bowl_11_15_overs_contribution_score, 
      0
    )
  ) as bowl_11_15_overs_contribution_score, 
  sum(
    coalesce(
      bowl_deathovers_contribution_score, 
      0
    )
  ) as bowl_deathovers_contribution_score, 
  sum(
    coalesce(
      bat_powerplay_contribution_score, 
      0
    )
  ) as bat_powerplay_contribution_score, 
  sum(
    coalesce(
      bat_7_10_overs_contribution_score, 
      0
    )
  ) as bat_7_10_overs_contribution_score, 
  sum(
    coalesce(
      bat_11_15_overs_contribution_score, 
      0
    )
  ) as bat_11_15_overs_contribution_score, 
  sum(
    coalesce(
      bat_deathovers_contribution_score, 
      0
    )
  ) as bat_deathovers_contribution_score, 
  sum(
    coalesce(
      overall_powerplay_contribution_score, 
      0
    )
  ) as overall_powerplay_contribution_score, 
  sum(
    coalesce(
      overall_7_10_overs_contribution_score, 
      0
    )
  ) as overall_7_10_overs_contribution_score, 
  sum(
    coalesce(
      overall_11_15_overs_contribution_score, 
      0
    )
  ) as overall_11_15_overs_contribution_score, 
  sum(
    coalesce(
      overall_deathovers_contribution_score, 
      0
    )
  ) as overall_deathovers_contribution_score, 
  sum(actual_powerplay_over_runs) as actual_powerplay_over_runs, 
  sum(actual_7_10_over_runs) as actual_7_10_over_runs, 
  sum(actual_11_15_over_runs) as actual_11_15_over_runs, 
  sum(actual_death_over_runs) as actual_death_over_runs, 
  sum(batting_strike_rate) as batting_strike_rate, 
  sum(
    case when coalesce(runs_scored, 0)>= 30 then 1 else 0 end
  ) as total_thirty_plus_scores, 
  sum(case when is_out = 0 then 0 else 1 end) as dismissal, 
  sum(
    coalesce(
      cast(total_balls_bowled / 6 as int) + cast(total_balls_bowled % 6 as decimal)/ 10, 
      0
    )
  ) as total_overs_bowled, 
  sum(
    coalesce(total_balls_bowled, 0)
  ) as total_balls_bowled, 
  sum(
    coalesce(runs_conceded, 0)
  ) as runs_conceded, 
  sum(
    case when coalesce(wickets_taken, 0)>= 3 then 1 else 0 end
  ) as total_three_plus_wickets, 
  sum(
    coalesce(
      bat_consistency_score_powerplay, 
      0
    )
  ) as bat_consistency_score_powerplay, 
  sum(
    coalesce(bat_consistency_score_7_10, 0)
  ) as bat_consistency_score_7_10, 
  sum(
    coalesce(bat_consistency_score_11_15, 0)
  ) as bat_consistency_score_11_15, 
  sum(
    coalesce(
      bat_consistency_score_deathovers, 
      0
    )
  ) as bat_consistency_score_deathovers, 
  sum(
    coalesce(
      bowl_consistency_score_powerplay, 
      0
    )
  ) as bowl_consistency_score_powerplay, 
  sum(
    coalesce(bowl_consistency_score_7_10, 0)
  ) as bowl_consistency_score_7_10, 
  sum(
    coalesce(bowl_consistency_score_11_15, 0)
  ) as bowl_consistency_score_11_15, 
  sum(
    coalesce(
      bowl_consistency_score_deathovers, 
      0
    )
  ) as bowl_consistency_score_deathovers 
from 
  contribution_data
where load_timestamp = '{str(load_timestamp)}' 
group by 
