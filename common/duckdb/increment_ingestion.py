...
from DataService.utils.helper import defaulting_image_url
from common.duckdb.increment_ingestion_query import GET_PRESSURE_INDEX
...
logger = get_logger("boot_script", "duck_flush_increment_insert_module")


def incremental_ingestion(connector_object):
    from common.duckdb.increment_ingestion_query import GET_SEASONS_DATA, GET_MATCHES_DATA, GET_PLAYERS_DATA, \
        ...
        MATCHES_JOIN_DATA, \
        BATSMAN_STATS, BOWLER_STATS, GET_CONTRIBUTION_SCORE_DATA, GET_AGG_CONTRIBUTION_DATA
    from common.duckdb.increment_ingestion_query import GET_TEAMS_DATA
    ...
    logger.info(f"increment_ingestion.py image folder_name: {folder_name}")
    defaulting_image_url(players_data, 'player_image_url', 'competition_name', 'WPL', folder_name)
    logger.info("increment_ingestion.py image defaulting completed")
    ...
    bowl_card_data = getPandasFactoryDF(session, GET_BOWL_CARD_DATA)
    bowl_card_data[[
        'overs', 'economy'
    ]] = bowl_card_data[[
        'overs', 'economy'
    ...
    pi_data = getPandasFactoryDF(session, GET_PRESSURE_INDEX)
    ...
    match_data_df1 = getPandasFactoryDF(session, MATCH_PLAYER_SQL).explode('team1_players').drop('team2_players',
                                                                                                 axis=1)

    match_data_df2 = getPandasFactoryDF(session, MATCH_PLAYER_SQL).explode('team2_players').drop('team1_players',
                                                                                                 axis=1)

    match_data1 = psql.sqldf('''select match_id, season, 1 as innings, team1, team1_players as player_id, venue, match_date, match_time,
            case when team1 = winning_team then 'Winning' when winning_team=-1 then 'No Result' else 'Losing' end as winning_type,
            team2, case when team1 = winning_team then 'WIN' when winning_team=-1 then 'NO RESULT' else 'LOSS' end as match_decision
            , competition_name from match_data_df1''')

    match_data2 = psql.sqldf('''select match_id, season, 2 as innings, team2 as team1, team2_players as player_id, venue, match_date,
            match_time, case when team2 = winning_team then 'Winning' when winning_team=-1 then 'No Result' else 'Losing'
            end as winning_type, team1 as team2, case when team2=winning_team then 'WIN' when winning_team=-1 then 'NO RESULT'
            else 'LOSS' end as match_decision, competition_name  from match_data_df2''')
    ...
    col_list = ['overall_powerplay_contribution_score', 'overall_7_10_overs_contribution_score',
                'overall_11_15_overs_contribution_score', 'overall_deathovers_contribution_score',
                'overall_consistency_score', 'overall_contribution_score', 'bowl_powerplay_contribution_score',
                'bowl_7_10_overs_contribution_score', 'bowl_11_15_overs_contribution_score',
                'bowl_deathovers_contribution_score', 'bowling_contribution_score', 'bowling_consistency_score',
                'bat_powerplay_contribution_score', 'bat_7_10_overs_contribution_score', 'bowl_innings',
                'bat_11_15_overs_contribution_score', 'bat_deathovers_contribution_score', 'wickets_taken',
                'batting_contribution_score', 'batting_consistency_score', 'runs_scored', 'bat_innings',
                'actual_powerplay_over_runs', 'actual_7_10_over_runs', 'actual_11_15_over_runs',
                'actual_death_over_runs', 'balls_faced', 'batting_strike_rate', 'runs_conceded',
                'total_overs_bowled', 'overall_fours', 'overall_sixes', 'fow_during_stay', 'non_striker_runs',
                'total_balls_bowled', 'arrived_on']
