...
def player_mapper():
    ...
    player_mapping_df = getPandasFactoryDF(
        session,
        f"select cricsheet_id from {PLAYER_MAPPER_TABLE_NAME}"
    )
    cricsheet_player_mapping_df = player_mapping_df[['cricsheet_id']]
    ...
    for row in data:
        if cricsheet_player_mapping_df.isin([str(row)]).any()[0]:
            continue
        print("misisng -> ", str(row))
        is_wicket_keeper = 0
        is_bowler = 0
        is_batsman = 0
        try:
        ...
        player_mapper_list.append(
            {
                ...
                'source_name': (str(data[row]['Name'])).replace("'", "''"),
                'is_bowler': is_bowler,
