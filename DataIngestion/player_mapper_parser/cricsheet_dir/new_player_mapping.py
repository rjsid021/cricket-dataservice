...
def update_new_players():
    ...
    existing_players_mapping = getPandasFactoryDF(session, f"select cricinfo_id from {DB_NAME}.{PLAYER_MAPPER_TABLE_NAME}")
    player_mapper_list = []
    iterator = 1
    people_file.dropna(subset=['key_cricinfo'], inplace=True)
    people_file['key_cricinfo'] = people_file['key_cricinfo'].astype(int)
    existing_players_mapping.dropna(subset=['cricinfo_id'], inplace=True)
    existing_players_mapping = existing_players_mapping[existing_players_mapping['cricinfo_id'] != '']
    existing_players_mapping['cricinfo_id'] = existing_players_mapping['cricinfo_id'].astype(int)
    # people_file['key_cricinfo'] = people_file['key_cricinfo'].astype(int)
    people_file = people_file[~(people_file['key_cricinfo'].isin(existing_players_mapping['cricinfo_id']))]
